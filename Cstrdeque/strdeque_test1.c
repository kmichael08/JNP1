...
int main() {
    ...
    unsigned long d4 = strdeque_new();

    assert(strdeque_size(d4) == 0);

    strdeque_insert_at(d4, 0, "heura");
    strdeque_insert_at(d4, 2, "straszna");

    assert(strdeque_size(d4) == 2);

    assert (strdeque_comp(d4, d4) == 0);


    assert(streq(strdeque_get_at(d4, 1), "straszna", 8));
