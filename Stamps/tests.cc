...
//koniec Merge
	...
	Rectangles zestaw {Rectangle(1, 2), Rectangle(3, 5)};

	Rectangle patola = zestaw[1];

	// Mergujemy

	Rectangles z {Rectangle(3, 1, Position(1, 1)), Rectangle(3, 44, Position(1, 2))};

	Rectangle wyn = merge_horizontally(z[0], z[1]);

	Rectangles z2 {Rectangle(3, 3, Position(1, 1)), Rectangle(1, 3, Position(4, 1))};

	Rectangle wyn2 = merge_vertically(z2[0], z2[1]);

	// Splitujemy

	z2.split_horizontally(0, 1);

	for (int i = 0; i < 3; i++)
		std::cout << z2[i].width() << " " << z2[i].height() << std::endl;


	/*
	Rectangle rpat(-1, 0);

	std::pair<Rectangle, Rectangle> para = rpat.split_horizontally(110);

	std::pair<Rectangle, Rectangle> para2 = rpat.split_vertically(101);
